...
- `-e --id` は**パッケージ ID の完全一致**指定。部分一致や「複数一致」の確認を避けられ、確実・再現性が高い。
- これは**公式のスタンドアロンインストーラを自動でダウンロード・インストール**するので、
  QGIS 本体・GDAL・OSGeo4W Shell が一式入る（手動ダウンロード版と中身は同じ）。
- 最新版が良ければ `winget install -e --id OSGeo.QGIS`（2026 時点 4.0 系）。初めてなら LTR を推奨。
...
スタートメニューに **「QGIS 3.44.11」というフォルダ名（バージョン番号付き）**ができ、その中に以下が入っている：
- `QGIS Desktop 3.44.11` … GUI アプリ本体
- **`OSGeo4W Shell`** … GDAL コマンドを打つための専用プロンプト（今回使う）

> **スタートメニューに見当たらないとき**：「QGIS」という単体項目ではなく **「QGIS 3.44.11」フォルダ**として入る（Q の位置）。検索ボックスに `OSGeo4W` か `QGIS` と打てば出る。インストール直後は検索インデックスが追いつかず一覧に出ないこともあり、その場合はサインアウト/再起動で表示される。今すぐ起動したいなら `Win + R` に次を貼って Enter：
> ```
> "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\QGIS 3.44.11\OSGeo4W Shell.lnk"
> ```
