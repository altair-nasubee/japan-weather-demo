...
**Status:** ✅ 実装完了（2026-06-30）。Task 1〜5 すべて完了。光柱マーカーはビルボードピンに置き換え済み。
...
- 既存 EditMode テストは本計画完了時点で **46/46** green を維持する。
...
- [x] **Step 3: Unity に取り込み Sprite として設定する**
...
- [x] **Step 5: 全 EditMode テストを確認する**

Unity MCP `run_tests`（EditMode 全件）。
Expected: **46/46** PASS（`BillboardScaleTests` 2 件追加後の総数）。
...
- [x] **Step 1: CityMarker.cs を全面書き換え**
...
- [x] **Step 3: 全 EditMode テストを確認する**

Unity MCP `run_tests`（EditMode 全件）。
Expected: **46/46** PASS（書き換えで既存テストが壊れていないこと）。
...
- [x] **Step 1: BuildMarkers にカメラ注入を追加**
...
- [x] **Step 4: MainScene の MapManager 参照を確認**
...
- [x] EditMode テスト **46/46** green（`BillboardScale` 2 件を含む）。
- [x] Play で Task 5 Step 5 の 1〜6 をすべて満たす。
- [x] 光柱（emissive 円柱 / 選択時 point light）が完全に除去されている。
- [x] ドロップダウン双方向同期・カメラフォーカスが従来どおり動作（無改修）。
