...
現在、右上の情報パネルが選択中の都市名（「東京」など）を **テキスト表示** しているだけで、都市の切り替えは地図上のマーカークリックに限られている。これを **ドロップダウンリスト** に置き換え、リストから各地点を選択できるようにする。さらに、選択した地点へ **カメラがスムーズにズームジャンプ** し、その地点の天候エフェクト（空・雲・降水）が見やすい構図へ寄る。
